...
這個實驗程式 (`experiment_systematic.py`) 實作了系統抽樣（Systematic Sampling）方法，並跟標準的有放回隨機抽樣做比較。

系統抽樣是一種改進的抽樣技術，在 frequent itemset mining 中可以得到更穩定、更準確的結果。
...
frequent itemset mining 的資料通常符合這些特性。
...
不會。起點是隨機的，只是間隔固定。這樣保留了隨機性，同時減少了方差。

### Q2: 為什麼不直接用分層抽樣？

系統抽樣更簡單，不需要預先定義分層。它會自動根據機率分布做隱式分層。

### Q3: 所有資料集都適用嗎？

大多數情況下都有改善。如果資料完全隨機無結構，兩者效果相近。

### Q4: 可以用在其他應用嗎？

可以。系統抽樣廣泛用於調查抽樣、品質控制、生產線檢測等領域。
